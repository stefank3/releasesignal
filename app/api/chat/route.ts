...
function buildWorkflowActionMessage(args: {
  ...
  if (args.workflowAction === "regenerate_suite") {
    ...
    return [
      ...
      "Improve the existing suite as a preservation-first replacement version.",
      "Preserve all valuable existing coverage from the current suite.",
      "Do NOT compress the suite merely to make it shorter or cleaner.",
      "Do NOT collapse distinct positive, negative, edge, role-based, integration, state-transition, or failure-mode scenarios into one generic test.",
      "Remove or consolidate cases only when they are clearly duplicate, invalid, malformed, or materially repetitive.",
