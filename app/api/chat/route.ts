...
if (!sessionId) {
  // IDP: sessionClientId is generated by the client when the user starts a "New chat".
  // Using upsert prevents duplicates from double-clicks, retries, or concurrent POSTs.
  const rawClientId = typeof body?.sessionClientId === "string" ? body.sessionClientId.trim() : "";

  // WHY: fallback makes the API resilient if frontend hasn’t been updated yet.
  // Best practice: frontend should always send sessionClientId for new sessions.
  ...
  const sessionRow = await prisma.chatSession.upsert({
    ...
    update: {
      // UX: if the same "new chat" key is reused, keep it stable;
      // optionally update title only if currently null (avoid overwriting user renames)
      title: undefined,
    },
    select: { id: true },
  });
