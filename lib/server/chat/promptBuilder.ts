...
function buildCasesModeInstruction(args: {
  ...
  return [
    ...
    "Explicit user exclusions are authoritative: never reintroduce excluded domains, examples, systems, behaviors, or assumptions in any case section.",
    "Do not invent downstream systems, integrations, logging, retries, partial-failure behavior, side effects, or lifecycle behavior unless present in the source requirement.",
    ...
    "Pressure-test idempotency, state transitions, downstream side effects, and recovery behavior only when relevant to source-provided details and not explicitly excluded.",
...
function buildCoachModeInstruction(args: {
  ...
  return [
    ...
    "Explicit user exclusions are authoritative: never reintroduce excluded domains, examples, systems, behaviors, or assumptions as objective, scope, rules, criteria, risks, constraints, coverage targets, minimal repro scenarios, or open questions.",
